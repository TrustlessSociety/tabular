...
function booleanEditor(column: GridColumn): NonNullable<ColumnDefinition['editor']> {
  return (cell, onRendered, success, cancel) => {
    ...
    input.setAttribute('aria-label', `Edit ${column.label || column.coordinate}`);
    /**
     * Finish the switch editor once with its current logical value.
     */
    const finish = (value: GridCellValue) => {
      if (settled) return;
      settled = true;
      success(value);
    };
    input.addEventListener('change', () => finish(input.checked));
    input.addEventListener('blur', () => {
      finish(switchFieldExitValue(cell.getValue()));
    }, { once: true });
    input.addEventListener('keydown', (event) => {
      if (event.key === 'Escape') {
...
/**
 * Return an empty edit token when a blank switch exits without a selection.
 */
export function switchFieldExitValue(value: unknown): GridCellValue {
