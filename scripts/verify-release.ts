...
for (const script of [
  ...
  assert.match(
    ...
    /^node scripts\/local-review\/run\.mjs /,
    `${script} must use the plain-Node local-review runner`
  );
  assert.doesNotMatch(
    packageJson.scripts[script]!,
    /(?:^|\s)(?:tsx|--loader|--import\s+tsx)(?:\s|$)/,
...
const pluginEntries = (await fs.readdir(path.join(projectRoot, 'plugins'), {
  withFileTypes: true
})).filter((entry) => entry.isDirectory()).map((entry) => entry.name).sort();
const registered = packageJson.plugins.map((entry) => entry.split('/')[2]).sort();
assert.deepEqual(registered, pluginEntries, 'Every feature plugin must be registered exactly once');
for (const plugin of pluginEntries) {
  const entries = await fs.readdir(path.join(projectRoot, 'plugins', plugin), {
    withFileTypes: true
  });
  for (const entry of entries) {
    if (!entry.isDirectory()) continue;
    assert.ok(
      (await fs.readdir(path.join(projectRoot, 'plugins', plugin, entry.name))).length > 0,
...
const releaseRunner = await fs.readFile(
  path.join(projectRoot, 'scripts/release/run-release-readiness.ts'),
  'utf8'
);
