...
/** Returns one absolute compiled entrypoint path. */
function entrypoint(name: 'web' | 'worker' | 'migrate') {
  return path.join(projectRoot, 'dist/entrypoints', `${name}.js`);
}
...
/** Confirms setup produced every compiled entrypoint before any process starts. */
async function assertBuiltEntrypoints() {
  for (const name of ['web', 'worker', 'migrate'] as const) {
    const stat = await fs.stat(entrypoint(name)).catch(() => undefined);
    if (!stat?.isFile()) {
      throw new Error('Compiled entrypoints are missing; run npm run local-review:setup first');
    }
  }
}
