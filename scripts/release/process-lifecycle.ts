...
try {
  ...
  const preflight = await run('node', ['dist/entrypoints/preflight.js'], {
    ...productionBase,
  ...
  const migration = await run('node', ['dist/entrypoints/migrate.js'], {
    ...productionBase,
  ...
  const doctor = await run('node', ['dist/entrypoints/doctor.js', '--scope', 'web'], {
    ...productionBase,
  ...
  const firstSeed = await run(
    'node',
    ['dist/entrypoints/seed-demo.js', '--confirm-local-demo'],
    seedEnvironment
  );
  ...
  await run('node', ['dist/entrypoints/seed-demo.js', '--confirm-local-demo'], seedEnvironment);
  ...
  for (const iteration of ['a', 'b']) {
    const web = start('node', ['dist/entrypoints/web.js', '--port', '0'], {
      ...productionBase,
      TABULAR_INSTANCE_ID: `web-${iteration}`,
      TABULAR_WEB_DATABASE_URL: urls.web
    });
    const worker = start('node', ['dist/entrypoints/worker.js'], {
      ...productionBase,
