...
//client
import type { ApplicationRuntimeService } from '../../src/bootstrap/application.js';
import { prepareProductPage, renderRegisteredView } from '../../src/plugins/app/helpers/rendering.js';
import { registeredViewEntries } from '../../src/bootstrap/build.js';

const root = process.cwd();

test('every page file is one default-exported lazy entry and plugin roots do not eagerly import it', async () => {
  const pageFiles = await pageFilesUnder(path.join(root, 'src/plugins'));
  ...
  ].map((plugin) => fs.readFile(path.join(root, 'src/plugins', plugin, 'plugin.ts'), 'utf8')));
...
test('build discovery reads registered views once and never calls a lazy page handler', () => {
  ...
  const views = new Map([
    ['/pages/table.html', new Set([{ entry: '@/src/plugins/grid/views/table', priority: 0 }])],
    ['/pages/browse.html', new Set([{ entry: '@/src/plugins/explorer/views/index', priority: 0 }])],
    ['/pages/', new Set([{ entry: '@/src/plugins/grid/views/table', priority: 0 }])]
  ]);
  const entries = registeredViewEntries({ views } as never);
  assert.deepEqual([...entries], [
    '@/src/plugins/grid/views/table',
    '@/src/plugins/explorer/views/index'
  ]);
...
test('page response data flows to its paired view without widening the Provider projection', async () => {
  ...
  assert.equal(rendered?.entry, '@/src/plugins/grid/views/table');
...
test('each rendered route registers exactly one feature-owned view entry', async () => {
  const expected: Array<[string, string, string]> = [
    ['explorer', '/', '@/src/plugins/explorer/views/index'],
    ['explorer', '/pages/browse.html', '@/src/plugins/explorer/views/index'],
    ['grid', '/pages/table.html', '@/src/plugins/grid/views/table'],
    ['identity', '/auth/login', '@/src/plugins/identity/views/login'],
    ['identity', '/auth/account', '@/src/plugins/identity/views/account'],
    ['import-export', '/pages/import.html', '@/src/plugins/import-export/views/import'],
    ['operations', '/pages/system-activity.html', '@/src/plugins/operations/views/activity']
  ];
  for (const [plugin, route, entry] of expected) {
    const source = await fs.readFile(path.join(root, 'src/plugins', plugin, 'plugin.ts'), 'utf8');
