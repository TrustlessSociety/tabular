...
} from '../../../src/plugins/grid/helpers/contracts.js';
import type { TabulatorTableFactory, TabulatorTablePort } from '../../../src/plugins/grid/helpers/tabulator-adapter.js';
import {
  borderBackgroundLayers,
  presentationNumberDisplay,
  switchFieldExitValue,
  TabulatorGridAdapter
} from '../../../src/plugins/grid/helpers/tabulator-adapter.js';
...
test('a blank Switch editor emits a field-exit token without choosing No', () => {
  assert.equal(switchFieldExitValue(null), '');
  assert.equal(switchFieldExitValue(undefined), '');
  assert.equal(switchFieldExitValue(false), false);
});
...
test('adapter definitions cover all ten accepted field editors and output formatters', async () => {
  ...
  const switchCell = (value: GridCellValue) => ({
    getValue: () => value,
    getField: () => 'switch',
    getRow: () => ({
      getData: () => ({ id: 'switch-output' })
    }) as unknown as RowComponent
  }) as unknown as CellComponent;
  assert.equal(
    switchFormatter(switchCell(null), {}, () => undefined),
    '',
    'an empty Switch cell stays visually empty'
  );
  assert.equal(switchFormatter(switchCell(false), {}, () => undefined), 'No');
