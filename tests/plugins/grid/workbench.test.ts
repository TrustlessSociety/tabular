...
//client
import type { CommandContext } from '../../../src/plugins/commands/helpers/contracts.js';
import type { IconName } from '../../../src/plugins/app/components/icon.js';
import { GridCanvas, displaySelection } from '../../../src/plugins/grid/components/grid-canvas.js';
import { REVIEW_COLUMNS, createReviewRows } from './fixtures.js';
import { SpreadsheetMenuBar } from '../../../src/plugins/commands/components/command-surface.js';
import { Icon } from '../../../src/plugins/app/components/icon.js';
import { SelectionInspector } from '../../../src/plugins/grid/components/selection-inspector.js';
import { EmphasisButton } from '../../../src/plugins/app/components/emphasis-button.js';
import { spreadsheetColumns } from '../../../src/plugins/grid/views/table.js';
...
test('a blank spreadsheet exposes the default A through Z coordinate range', () => {
  const columns = spreadsheetColumns([]);
  assert.equal(columns.length, 26);
  assert.equal(columns[0]?.coordinate, 'A');
  assert.equal(columns.at(-1)?.coordinate, 'Z');
});
