...
try {
  ...
  // Build browser artifacts once, then execute checked-in TypeScript entrypoints.
  await runVisible('npm', ['run', 'build']);
  const migrate = sourceRuntime('migrate');
  await runVisible(
    process.execPath,
    [...migrate.args, migrate.entrypoint],
    localReviewEnvironment()
  );
  await grantRuntimeAuthority();
  const seed = sourceRuntime('seed-demo');
  await runVisible(
    process.execPath,
    [
      ...seed.args,
