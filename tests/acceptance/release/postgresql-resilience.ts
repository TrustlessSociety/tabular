...
const runtimeArgs = (name: string, ...args: string[]) => [
  tsxCli,
  `scripts/runtime/${name}.ts`,
  ...args
];
const outputRoot = path.join(projectRoot, 'tests/acceptance/evidence/release/task-00014');
...
try {
  ...
  process.stdout.write(`${JSON.stringify({
    result: 'passed',
    crashRecovery: 'tests/acceptance/evidence/release/task-00014/postgresql-crash-recovery.json',
    physicalRestore: 'tests/acceptance/evidence/release/task-00014/physical-backup-restore.json',
    backupBytes
  }, null, 2)}\n`);
} finally {
...
async function provePackagedReadiness(webUrl: string) {
  const child = spawn(process.execPath, runtimeArgs('web', '--port', '0'), {
    cwd: packageRoot,
    env: {
...
async function runPackage(entrypoint: string, values: NodeJS.ProcessEnv, args: string[] = []) {
  await new Promise<void>((resolve, reject) => {
    const child = spawn(process.execPath, runtimeArgs(entrypoint, ...args), {
      cwd: packageRoot,
