(function () {
  ...
  function initFileExplorer() {
    ...
    var requestedTab = params.get("tab") === "views" ? "views" : "files";
    var department = currentDepartment(params);
    var activeCollection = requestedFolder === "operations" || requestedFolder === "finance"
      ? department.key + (requestedTab === "views" ? "-views" : "")
      : "root";
    var collections = queryAll("[data-explorer-collection]", explorer);
    var count = query("[data-explorer-count]", explorer);
    var rootTitle = query("[data-explorer-root-title]", explorer);
    var tabs = query("[data-explorer-tabs]", explorer);
    var tabLinks = queryAll("[data-explorer-tab]", explorer);
    ...
    var emptyTitle = query("[data-explorer-empty-title]", explorer);
    var emptyCopy = query("[data-explorer-empty-copy]", explorer);
    var view = "list";
    ...
    function refreshSearch() {
      ...
      if (count) {
        var noun = activeCollection === "root" ? "folders" : (requestedTab === "views" ? "views" : "files");
    ...
    if (activeCollection === "root") {
      ...
      if (rootTitle) rootTitle.hidden = false;
      ...
      if (emptyTitle) emptyTitle.textContent = "No matching files";
      if (emptyCopy) emptyCopy.textContent = "Try a different folder or spreadsheet name.";
    } else {
      document.title = (requestedTab === "views" ? department.label + " views" : department.label) + " · Acme Inc.";
      if (rootTitle) rootTitle.hidden = true;
      if (tabs) tabs.hidden = false;
      if (folderCrumb) folderCrumb.hidden = false;
      if (folderCrumbText) folderCrumbText.textContent = department.label;
      tabLinks.forEach(function (link) {
        var tab = link.getAttribute("data-explorer-tab");
        link.setAttribute("aria-selected", String(tab === requestedTab));
        link.setAttribute("href", "./browse.html?folder=" + department.key + (tab === "views" ? "&tab=views" : ""));
      });
      if (newFile) {
      ...
      if (search) {
        search.placeholder = requestedTab === "views" ? "Search views" : "Search files";
        search.setAttribute("aria-label", requestedTab === "views" ? "Search views" : "Search files");
      }
      if (emptyTitle) emptyTitle.textContent = requestedTab === "views" ? "No matching views" : "No matching files";
      if (emptyCopy) emptyCopy.textContent = requestedTab === "views" ? "Try a different view or spreadsheet name." : "Try a different folder or spreadsheet name.";
    }
  ...
  function runSheetCommand(command) {
    ...
    if (command === "undo") {
      ...
      var exportViewLabels = { "follow-ups": "My follow-ups", ready: "Ready to ship", unpaid: "Unpaid orders", overdue: "Overdue invoices", unreviewed: "Needs review" };
      var exportView = new URLSearchParams(window.location.search).get("view");
      showToast("Export ready", (exportViewLabels[exportView] || "Current sheet") + " will be exported as CSV.");
