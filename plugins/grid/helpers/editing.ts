...
export function stageCellEdit(
  ...
  return {
    ...
    kind: 'cells',
    source: 'edit',
    changes: [draftChange(point, row[point.columnId] ?? null, attempted, column)]
  };
}
...
export function stageInsertRow(
  ...
  for (const column of columns) {
    ...
    changes.push(draftChange(
      ...
      null,
      value,
      { ...column, editable: true, generated: false, required: column.serverDefault ? false : column.required }
    ));
  }
  return {
    id,
...
export function clearInsertDraftSelection(
  ...
  return points.reduce((current, point) => (
    point.rowId === current.row.id && draftColumnIds.has(point.columnId)
      ? updateInsertDraft(current, columns, point, null)
      : current
  ), draft);
}
...
export function insertDraftIsEmpty(
  ...
  return draft.changes.every((change) => {
    ...
    if (column?.key && /^NEW-\d+-[A-Z]+$/.test(change.raw)) return true;
    return column?.defaultValue !== undefined
      && String(column.defaultValue) === change.raw;
  });
}
...
export function capabilityActionForDraft(
  ...
  if (draft.kind === 'insert') {
    return {
      ...
      fileId: input.fileId,
      patch: draft.changes
        .filter((change) => !change.issue && change.after !== null)
        .map((change) => ({
  ...
  if (!draft.rowId) {
    ...
    let changes = columns.flatMap((selected) => {
      ...
      const attempted = entry
        ? gridValueFromTyped(entry.value)
